...
void write_char(const char *p) {
  de_printf("THIS SHOULDNT EXISTRRRRRRRRRR");
  return;
  ...
  //   buf[0] = *p;
  //   buf[1] = '\0';
  //   de_printf("What's the p pointer: %c\n", *p);
  ...
  //
  //   // de_printf("****** write call, written is: %zd\n", written);
  //   // de_printf("And what the heck did I actually write?: %s\n", buf);
  //
  ...
  //   font->ascent, &r, 1); XftGlyphFontSpec spec; spec.font = font; spec.glyph
  //   = glyph; spec.x = x; spec.y = y;
  //
